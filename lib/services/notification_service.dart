...
class NotificationService {
  ...
  // 5. Uygulama ön plandayken FCM bildirimini göstermek için
  Future<void> showNotificationNow({
    ...
    const AndroidNotificationDetails androidDetails =
        AndroidNotificationDetails(
          'exam_channel_id', // Mevcut kanal ID'sini kullanıyoruz
    ...
    if (Platform.isIOS) {
      ...
      if (androidImplementation != null) {
        ...
        try {
          ...
          print(
            "requestExactAlarmsPermission metodu bulunamadı veya hata verdi: $e",
          );
        }
      }
    }
  ...
  Future<void> scheduleExamNotification({
    ...
    final tz.TZDateTime finalNotificationTime = tz.TZDateTime.from(
      localTime,
      tz.local,
    );
    ...
    if (finalNotificationTime.isBefore(now.add(const Duration(seconds: 10)))) {
      print(
        "Bildirim zamanı geçmişte veya çok yakında. Planlama iptal edildi.",
      );
      return;
    }
    ...
    print(
      "✅ Bildirim planlandı: ID $notificationId, Zaman: ${finalNotificationTime.toLocal()}",
    );
  }
